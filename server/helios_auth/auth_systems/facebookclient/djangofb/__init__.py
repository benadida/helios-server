    ...
    def decorator(view):
        def newview(request, *args, **kwargs):
            ...
            if callable(next):
                ...
                next is None
                and fb.callback_path
                and request.path.startswith(fb.callback_path)
            ):
    ...
    def process_request(self, request):
        ...
        if not self.internal:
            if "fb_sig_session_key" in request.GET and "fb_sig_user" in request.GET:
                ...
                ] = request.GET["fb_sig_session_key"]
                request.facebook.uid = request.session["fb_sig_user"] = request.GET[
                    "fb_sig_user"
                ]
            elif request.session.get(
                "facebook_session_key", None
