...
Greece. Based on its good track record and the body of work that has
...
In section~\ref{sec:experience} we relate our experiences with
...
Each poll in Zeus has the following phases:
...
the provided answers.\footnote{
    See Sections~\ref{sec:ballot_encoding} and~\ref{sec:approval_counting}
    for details on the types of elections supported.}
Then, the administrator uploads the list of voters that may participate
in the poll.\footnote{a simple CSV format is used, easily exported from
spreadsheets.}

Before the voting starts, each of the trustees must visit Zeus' website
...
invitation is send via e-mail as a secret link into Zeus' website.
However, it is easy for Zeus to be adapted to use any system for
designation and notification of trustees.\footnote{
    An adaptation for notifications via mobile phones was actually
    implemented but was never used or streamlined within the software.
}
See discussion in Section~\ref{sec:audit_codes}.
...
assisted with randomness downloaded from the server itself.\footnote{
due to poor randomness in browsers, a practice inherited from Helios.}
After key creation, the trustee is prompted to save his private key,
...
procuring it so that it can be compared to the one uploaded.
...
to them via e-mail.\footnote{
for \emph{audit} votes see Section~\ref{sec:audit_codes}}
...
may be added one by one using generated confidential links.\footnote{
Zeus provides a command line toolkit that, among other things, can
perform mixing given those confidential links.}

Finally, the administrator ends the mixing process\footnote{
details in Section~\ref{sec:mixing}},
and the trustees are notified again for the decryption process.
Each one visits the Zeus website through the invitation link.
The mixed encrypted ballots are downloaded to their client browser,
and they are prompted to procure their secret poll key.
When provided, the ballots are partially decrypted and the output
is sent back to Zeus.
...
system for tallying and reporting on the results,
...
within the candidate list which is interpreted at tallying time
...
by combining\footnote{by a multiplication, actually}
...
existing encoding.
Due to time constraints we used approval-to-ranking translation
for its simplicity and malleability.
\footnote{Now that the structure of this ballot type is more or less
          stable, we may consider a specialized encoding in the future.}
...
voters and of those who actually voted in each election. Elections
were carried out successfully in all planned institutions. This was a
...
itself in a request we received twice to provide to the election
committee a log of the exact times each voter cast a ballot. We
...
was before ballot closing time. It turned out that he was right, and
...
out to them when they communicated about their problem with the
respective election committee and was thereby resolved; we did receive
an angry e-mail, though, from a voter who felt slighted that she had
