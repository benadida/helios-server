...
\affil{Greek Research and Education Network (GRNET)}
KOSTAS PAPADIMITRIOU
\affil{Greek Research and Education Network (GRNET)}
PANOS LOURIDAS\symbolfootnote[1]{Corresponding author.}
\affil{Greek Research and Education Network (GRNET)}
PANAYIOTIS TSANAKAS
...
A related problem, indicated by one of the anonymous reviewers, is
that ``a voter who would like to compromise the privacy of another
voter can simply copy the ciphertext of that voter, maybe modify it a
bit in a controlled way using the homomorphic property of the
encryption scheme, and then look for duplicate ballots (or for a pair
of ballot with a difference matching the modification made on the
ciphertext) after decryption''. It was easy to guard against that, and
it was an oversight that we did not do it from the outset. To calm our
worries, we checked all past elections and verified that no such
chicanery occurred. We subsequently addressed this for any submited
ciphertext $(g^r, c_2)$ by including $c_2$ in the hash that produces
the challenge for the proof of knowledge of $r$. This way, the proof
of knowlegde is bound to the specific ciphertext and therefore is not
reusable.
...
At the point of this writing (June 2013), Zeus has been used in 60
elections around Greece. Figure~\ref{fig:elections_to_date} shows the
number of registered voters and the turnout in each election. The
registered voters in all elections totaled 15599, while the voters
that actually took part in the elections totaled 12171; Elections were
...
\begin{figure}[ht]
  ...
  \end{center}
  \caption{Zeus held elections to June 2013.}
