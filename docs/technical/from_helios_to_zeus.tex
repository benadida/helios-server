...
If a vote is submitted with an accompanying code, two things may
happen. If the code is among the ones
the voter has received, then the server considers it to be an
audit. If the code is not among those supplied by the server, then it is
...
but potentially unsecure means, for example the e-mail,
...
This speeded our exponentiations up about 7 times.
...
% especially after we discovered that primitives from widely available
% cryptographic libraries had flaws that could hurt us.
% % PyCrypto here?
% 
% For reference, our software dependencies for cryptography are
