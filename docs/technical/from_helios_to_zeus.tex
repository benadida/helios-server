...
The rest of this paper proceeds as follows. In section~\ref{related},
we discuss related work and the roots of Zeus. In
section~\ref{challenges}, we discuss Zeus's background and outline the
practical problems we faced and our approach to solve them. In
section~\ref{workflow}, we go through the complete voting workflow
from the users' perspective. In section~\ref{architecture}, we
...
view. In section~\ref{experiences}, we relate our experiences with
...
ballots for their anonymization. The use of mixnets was later dropped,
as we will see, in favour of 
...
developed by Ben Adida. Work on Helios has continued, and is currently
at version 3. The original publication described a system in which
anonymity was provided by the use of mixnets. Over time researchers
have explored possible vulnerabilities in different versions of
Helios~\cite{heiderich:2011}. To the best of our knowledge these
attacks are not applicable to Zeus. 
...
Zeus was based on version 3.1 of Helios, which uses homomorphic
tallying, but actually implements the mixing workflow proposed in the
initial publication~\cite{adida:2008}. Due to this and other
