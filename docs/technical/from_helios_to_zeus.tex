...
In section~\ref{sec:workflow} we go through the complete voting workflow
...
\subsubsection{Notifications to the Trustees}
...
\section{The Zeus Voting Workflow}
\label{sec:workflow}
...
as discussed in Section~\ref{ballot_counting}.
...
as detailed in Section~\ref{vote_preparation}.
...
\label{approval_counting}
In Section~\ref{ballot_encoding} we have established the encoding
used for ballots that select a subset of the candidates and
ranks each one in it, in a strict order.
