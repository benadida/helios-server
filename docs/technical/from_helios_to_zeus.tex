...
\item The system stores the vote as an audit vote.
\item The system asks the user to confirm that they have cast an
  audit vote.
\item If the user responds negatively, the audit process is
  canceled and the voter is redirected to the initial ballot casting
  view.
\item If the user responds in the affirmative, the browser sends
  to the system the ElGamal randomness with which the vote has
  been encrypted.
\item The system creates an audit vote record using the ciphertext and
  randomness provided by the browser and it returns to the voter a
  unique identifier of the audit ballot for future reference. 
