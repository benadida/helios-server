...
def show_all(request, receiver_id):
    shares_list = Signed_Encrypted_Share.objects.filter(
        receiver_id=receiver_id).order_by('signer_id')
...
def election_trustees_add_from_id(request, election_id, key_id):
    ...
    if election.frozen_trustee_list == False:
        trustee = Trustee(
            uuid=str(uuid.uuid1()), election=election, name=key.name, email=key.email, key=key)
        trustee.save()
...
def freeze_trustees_list(request, election_id):
    if request.method == 'POST':  # If the form has been submitted...
        ...
        template = loader.get_template(
            'bulletin_board/freeze_trustees_list.html')
        context = Context()
...
def encrypted_shares_home(request, election_id):
    election = Election.objects.filter(id=election_id)[0]
    shares_list = Signed_Encrypted_Share.objects.filter(
        election_id=election_id).order_by('signer_id')
