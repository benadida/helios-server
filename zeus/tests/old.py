...
from helios.models import FEATURES_REGISTRY
...
class FunctionalZeusTest(TestCase):
    ...
    def _random_polls(self, count=None):
        polls = []
        if not count:
            count = choice(range(1,50))

        for i in range(count):
            polls.append("Poll %d %s" % (i, random_sentence(3)))
    ...
    def create_random_election(self, admin):
        ...
        polls_url = '/helios/elections/%s/polls/' % uuid
        self.assertRedirects(r, polls_url)

        polls_data = self._random_polls()
    ...
    def freeze_election(self, admin, uuid):
        r = admin.post('/helios/elections/%s/freeze' % uuid,
                       {'csrf_token': admin.session.get('csrf_token')})

    def mix_election(self, admin, uuid):
        r = admin.post('/helios/elections/%s/compute_tally' % uuid,
                       {'csrf_token': admin.session.get('csrf_token')})

    def finish_mixing(self, admin, uuid):
        r = admin.post('/helios/elections/%s/stop-mixing' % uuid,
                       {'csrf_token': admin.session.get('csrf_token')})
    ...
    def trustees_decrypt(self, trustees):
        for trustee, kp in trustees.iteritems():
            ...
            r = client.post(
                '/helios/elections/%s/trustees/%s/upload-decryption' % (
                    t.election.uuid, t.uuid), {'factors_and_proofs':
                                               json.dumps(data)})
