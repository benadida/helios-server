...
class ElectionForm(forms.Form):
    name = forms.CharField(max_length=100, widget=forms.TextInput(
        attrs={'size': 60}), help_text='The pretty name for your election, e.g. My Club 2010 Election')
    short_name = forms.SlugField(
        label="Slug", max_length=25, help_text='No spaces, will be part of the URL for your election, e.g. my-club-2010')
    description = forms.CharField(max_length=2000, widget=forms.Textarea(
        attrs={'cols': 70, 'wrap': 'soft'}), required=False)
    election_type = forms.ChoiceField(
        label="Type", choices=Election.ELECTION_TYPES)
    use_voter_aliases = forms.BooleanField(
        required=False, initial=False, help_text='If selected, voter identities will be replaced with aliases, e.g. "V12", in the ballot tracking center')
    #use_advanced_audit_features = forms.BooleanField(required=False, initial=True, help_text='disable this only if you want a simple election with reduced security but a simpler user interface')
    randomize_answer_order = forms.BooleanField(
        required=False, initial=False, help_text='Enable this if you want the answers to questions to appear in random order for each voter')
    private_p = forms.BooleanField(required=False, initial=False, label="Private",
                                   help_text='A private election is only visible to registered voters.')
    use_threshold = forms.BooleanField(required=False, initial=False, label="Use threshold encryption",
                                       help_text='Using threshold encryption allows a subset of k out of n trustees to decrypt the tally')
    help_email = forms.CharField(required=False, label="Help e-mail address",
...
class ElectionTimesForm(forms.Form):
    ...
    voting_starts_at = SplitDateTimeField(
        help_text='UTC date and time when voting begins', widget=SplitSelectDateTimeWidget)
