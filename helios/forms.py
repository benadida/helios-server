...
class ElectionForm(forms.Form):
  ...
  #use_advanced_audit_features = forms.BooleanField(label=_("Use Advanced Audit Features"), required=False, initial=True, help_text=_('disable this only if you want a simple election with reduced security but a simpler user interface'))
  ...
  # times
  voting_starts_at = DateTimeLocalField(label=_("Voting Starts At"), help_text = _('UTC date and time when voting begins'),
                                   required=False)
  voting_ends_at = DateTimeLocalField(label=_("Voting Ends At"), help_text = _('UTC date and time when voting ends'),
                                   required=False)

class ElectionTimeExtensionForm(forms.Form):
  voting_extended_until = DateTimeLocalField(label=_("Voting Extended Until"), help_text = _('UTC date and time voting extended to'),
                                   required=False)

class EmailVotersForm(forms.Form):
  subject = forms.CharField(label=_("Subject"), max_length=80)
  body = forms.CharField(label=_("Body"), max_length=4000, widget=forms.Textarea)
...
class TallyNotificationEmailForm(forms.Form):
  subject = forms.CharField(label=_("Subject"), max_length=80)
  body = forms.CharField(label=_("Body"), max_length=2000, widget=forms.Textarea, required=False)
...
class VoterPasswordForm(forms.Form):
  voter_id = forms.CharField(label=_("Voter ID"), max_length=50)
