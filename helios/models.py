...
class EmailOptOut(models.Model):
  """
  Model to track email addresses that have opted out of all Helios emails.
  Stores hashed email addresses for privacy.
  """
  ...
  def is_opted_out(cls, email):
    ...
    Returns:
        Boolean indicating if email is opted out
    """
    from . import utils
  ...
  def add_opt_out(cls, email, user_agent=None, ip_address=None):
    ...
    Args:
        email: The email address to opt out
        user_agent: Optional user agent string
        ip_address: Optional IP address
        
    Returns:
        EmailOptOut instance (created or existing)
    """
    from . import utils
  ...
  def remove_opt_out(cls, email):
    """
    Remove an email from the opt-out list (opt back in).
    
    Args:
        email: The email address to opt back in
