...
def update_status(user_id, user_info, token, message):
    ...
    result = twitter_client.oauth_request(
        'http://api.twitter.com/1/statuses/update.json', args={'status': message}, method='POST')
...
def send_notification(user_id, user_info, message):
    ...
    result = twitter_client.oauth_request(
        'http://api.twitter.com/1/direct_messages/new.json', args={'screen_name': user_id, 'text': message}, method='POST')
